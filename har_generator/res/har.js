function fillEntries(adhoc, parentPage, perfEntries, imgs, documentURL, documentFileSize) {
  var entries = [],
    tdns = 0,
    tconnect = 0,
    tsend = 0,
...
//alert("Skipping "+rl[i].name+" initiator="+rl[i].initiatorType);
    ...
    httpStatus = "200";
    httpText = "OK";
    url = (rl[i].name == 'document') ? documentURL : rl[i].name;
...
function generateHAR() {
  ...
  var documentFileSize = document.fileSize;
  return generateHARNoDOM(timing, browserName, browserVersion, userAgent, documentTitle, entries, imgs, documentURL, documentFileSize);
}

function generateHARNoDOM(timing, browserName, browserVersion, userAgent, documentTitle, entries, imgs, documentURL, documentFileSize) {
  ...
  adhoc.log.browser.name = browserName;
  adhoc.log.browser.version = ""+browserVersion;
  adhoc.log.browser.comment = userAgent;
  ...
  adhoc.log.entries = fillEntries(adhoc, "page_0", entries, imgs, documentURL, documentFileSize);
  console.log(adhoc);
  return adhoc;
}

function postHAR(adhoc) {
